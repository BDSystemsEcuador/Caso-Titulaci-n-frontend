...
export class ConvocatoryListComponent implements OnInit {
  ...
  loadColsPlanning() {
        ...
        this.messageService.questionDelete({})
            .then((result) => {
                if (result.isConfirmed) {
                    ...
                    this.uicHttpService.delete('planning/delete', {ids: planning.id})
                        .subscribe(response => {
    ...
    removePlanning(planning: Planning) {
        this.planningsIn = this.planningsIn.filter(element => element !== planning);
    ...
    deletePlannings(planning = null) {
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          this.spinnerService.show();
                    this.uicHttpService.delete('planning/delete', ids)
                        .subscribe(response => {
                            this.spinnerService.hide();
                            this.messageService.success(response);
    ...
    searchPlannings(event, search) {
      if (event.type === 'click' || event.keyCode === 13 || search.length === 0) {
          ...
          this.spinnerService.show();
          this.uicHttpService.get('plannings', params).subscribe(response => {
              this.planningsIn = response['data'],
                  this.spinnerService.hide();
          }, error => {
              this.spinnerService.hide();
              this.messageService.error(error);
    ...
    removePlannings(ids) {
        for (const id of ids) {
            this.planningsIn = this.planningsIn.filter(element => element.id !== id);
        }
        this.planningsOut.emit(this.planningsIn);
    }
}